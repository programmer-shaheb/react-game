  ...
  webRtcPlayerObj.onDataChannelMessage = function (data) {
    ...
    if (view[0] === ToClientMessageType.QualityControlOwnership) {
      ...
      if (settingsJSON.WebRTC) {
        ...
        const webrtcDegradationPref = document.getElementById(
          "webrtc-degradation-pref"
        );
        if (webrtcDegradationPref) {
          webrtcDegradationPref.value = settingsJSON.WebRTC.DegradationPref;
        }

        const webrtcMaxFPSText = document.getElementById("webrtc-max-fps-text");
        if (webrtcMaxFPSText) {
          webrtcMaxFPSText.value = settingsJSON.WebRTC.MaxFPS;
        }

        const webrtcMinBitrateText = document.getElementById(
          "webrtc-min-bitrate-text"
        );
        if (webrtcMinBitrateText) {
          webrtcMinBitrateText.value = settingsJSON.WebRTC.MinBitrate / 1000;
        }

        const webrtcMaxBitrateText = document.getElementById(
          "webrtc-max-bitrate-text"
        );
        if (webrtcMaxBitrateText) {
          webrtcMaxBitrateText.value = settingsJSON.WebRTC.MaxBitrate / 1000;
        }

        const webrtcLowQPText = document.getElementById("webrtc-low-qp-text");
        if (webrtcLowQPText) {
          webrtcLowQPText.value = settingsJSON.WebRTC.LowQP;
        }

        const webrtcHighQPText = document.getElementById("webrtc-high-qp-text");
        if (webrtcHighQPText) {
          webrtcHighQPText.value = settingsJSON.WebRTC.HighQP;
        }
      }
    } else {
...
function onWebRtcAnswer(webRTCData) {
  ...
  webRtcPlayerObj.latencyTestTimings.OnAllLatencyTimingsReady = function (
    ...
    if (timings.FrameDisplayDeltaTimeMs && timings.BrowserReceiptTimeMs) {
      endToEndLatency =
        timings.FrameDisplayDeltaTimeMs + latencyExcludingDecode;
...
function connect() {
  ...
  var url = window.location.href
    .replace("http://", "ws://")
    .replace("https://", "wss://");
  url = "wss://ssi1.eaglepixelstreaming.com:4431/";
...
function receiveCommand(response) {
  var event = {};
  event.data = response;
  console.log("receiveCommand: " + JSON.stringify(event));
  //console.log('receiveCommand2: ' +JSON.stringify(event.data));
  //console.log(event);
  ...
  try {
    ...
    if (descriptor.Type != null) {
      console.log("Command received, parsed and accepted: %s", event.data);
      emitUIInteraction(descriptor);
    } else if (descriptor.cmd == "EmulatePlayButtonPressed") {
      onClickPlay();
    } else if (descriptor.cmd == "sendToUe4") {
      //console.log('descriptor.value: '+descriptor.value);
      emitUIInteraction(descriptor.value);
    } else if (descriptor.cmd == "ssitest") {
      //console.log('descriptor.value: '+descriptor.value);

      var jsonobj = {
        requestId: descriptor.requestId,
        jsonObjecived: JSON.stringify(descriptor),
      };

      emitUIInteraction(jsonobj);
    } else {
      console.log(
